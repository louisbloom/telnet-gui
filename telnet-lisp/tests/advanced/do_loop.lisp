...
(assert-equal (do ((i 0 (+ i 1))) ((>= i 10) i)) 10
 "simple counter with do loop")
;; Count up with side effects
(assert-equal (do ((i 1 (+ i 1))) ((> i 5) "done") i) "done"
 "do loop with side effects")
;; Multiple variables - count and accumulate sum
(assert-equal (do ((i 1 (+ i 1)) (sum 0)) ((> i 10) sum) (set! sum (+ sum i)))
 55 "do loop with multiple variables")
...
(assert-equal (do ((i 10 (- i 1))) ((<= i 0) "blastoff") i) "blastoff"
 "countdown with do loop")
;; Generate powers of 2 until > 1000
(assert-equal (do ((x 1 (* x 2))) ((> x 1000) x) x) 1024
...
(assert-equal (do ((count 0 (+ count 1))) ((>= count 5) count)) 5
