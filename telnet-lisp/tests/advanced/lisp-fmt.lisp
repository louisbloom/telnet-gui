...
(assert-equal (sexp-to-string '()) "nil" "empty list is nil")
...
(assert-equal (format-sexp '(quote (a b c)) 0) "'(a b c)" "format-sexp uses quote shorthand")

;; Quasiquote shorthand
(assert-equal (sexp-to-string '(quasiquote x)) "`x" "quasiquote outputs as backtick")
(assert-equal (sexp-to-string '(quasiquote (a b c))) "`(a b c)" "quasiquoted list")

;; Unquote shorthand
(assert-equal (sexp-to-string '(unquote x)) ",x" "unquote outputs as comma")
(assert-equal (sexp-to-string '(unquote-splicing x)) ",@x" "unquote-splicing outputs as comma-at")
...
;; Complex quasiquote
(assert-equal (sexp-to-string '(quasiquote (list (unquote a) (unquote-splicing b))))
  "`(list ,a ,@b)"
...
(assert-equal (format-sexp '(+ (* 2 3) 4) 0) "(+ (* 2 3) 4)" "nested expr on one line")
...
;; simple define
(assert-equal (format-sexp '(define x 42) 0)
  "(define x 42)"
  "simple define on one line")

;; Longer defun uses multi-line (via format-special-form)
;; Build a defun that's too long for one line
(assert-equal (format-sexp '(defun calculate-something-complex (first-param second-param)
...
(assert-equal (format-sexp '(let ((x 1)) x) 0)
  "(let ((x 1)) x)"
  "short let on one line")

;; Longer let* forces multi-line
(assert-equal (format-sexp '(let* ((first-variable 100) (second-variable 200))
...
(assert-equal (format-sexp '(if test then else) 0)
  "(if test then else)"
