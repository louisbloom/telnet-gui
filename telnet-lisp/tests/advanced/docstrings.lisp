...
(define single-expr (lambda (x) (+ x 1)))

(documentation 'single-expr) ; => nil

; Anonymous lambda with docstring
(define anon (lambda (x y) "Multiply X by Y." (* x y)))
...
(defmacro unless-test (condition . body) `(if ,condition nil (progn ,@body)))
...
(define return-string (lambda nil "Just a string"))
...
(define make-multiplier
  (lambda (factor) "Create a function that multiplies by FACTOR."
    (lambda (x) "Inner lambda - multiply X by captured factor." (* x factor))))
