...
(assert-equal (if v "not empty" "empty") "not empty"
 "non-empty vector is truthy")
(assert-equal (if empty "not empty" "empty") "not empty"
 "empty vector is truthy (only nil is falsy)")
(assert-equal (if empty1 "not empty" "empty") "not empty"
