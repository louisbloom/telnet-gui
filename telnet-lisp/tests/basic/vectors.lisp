...
(assert-equal (vector-length empty1) 0 "empty vector has length 0")

;; Create and manipulate vectors programmatically
(define v (make-vector 5))
(assert-equal (vector-length v) 0 "make-vector creates empty vector")

;; Add elements
(vector-push! v 10)
(vector-push! v 20)
(vector-push! v 30)
(assert-equal (vector-length v) 3 "vector has 3 elements after pushes")
...
(assert-equal (vector-length v-init) 5 "make-vector with initial value creates vector of correct length")
...
(assert-equal (vector-length v) 2 "vector length decreases after pop")
...
(assert-equal (vector-length empty) 0 "empty vector has length 0")
